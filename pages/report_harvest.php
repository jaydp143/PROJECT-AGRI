...
$pdf->AddPage();
   ...
   <table width ="100%">
       <tr>
          ...
          <br>
          '.$_GET['season'].' SEASON HARVEST REPORT '.$_GET['year'].'
          <br></td>
        </tr>
    </table>';
    ...
    //PANGASINAN
     ...
     (SELECT COALESCE(SUM(target), 0) FROM tbl_target as tr WHERE tr.program='HARVESTING' AND 		   tr.season='".$_GET['season']."' AND year='".$_GET['year']."' )target, 
     ...
     (((SELECT COALESCE(SUM(area), 0) FROM tbl_harvesting as pl  WHERE  date_monitored BETWEEN '".$startdate."' AND '".$enddate."')/(SELECT COALESCE(SUM(target), 0) FROM tbl_target as tr WHERE tr.program='HARVESTING' AND tr.season='".$_GET['season']."' AND year='".$_GET['year']."' ))*100)percentage");
     ...
     while($row1 = mysqli_fetch_array($queryP)){ 
       ...
       $tbl.='<td><b>'.number_format($row1['tyield'],2).'</b></td>';
       $tbl.='<td><b>'.number_format($row1['tproduction'],2).'</b></td>';
     ...
     (SELECT  COALESCE(SUM(target), 0) FROM tbl_target as tr INNER JOIN tbl_municipality as mn ON tr.mun_id=mn.mun_id WHERE tr.program='HARVESTING' AND tr.season='".$_GET['season']."' AND tr.year='".$_GET['year']."' AND mn.district='".$x."' )target,
     ...
     while($rowD = mysqli_fetch_array($queryD)){ 
       ...
       $tbl.='<td><b>'.number_format($rowD['yield'],2).'</b></td>';
       $tbl.='<td><b>'.number_format($rowD['production'],2).'</b></td>';
    ...
    (SELECT target FROM tbl_target as tr WHERE tr.mun_id=mn.mun_id AND tr.program='HARVESTING' AND tr.season='".$_GET['season']."' AND tr.year='".$_GET['year']."' )target,
    ...
    (SELECT COALESCE(SUM(yield), 0) FROM tbl_harvesting as pl WHERE pl.mun_id=mn.mun_id AND  date_monitored BETWEEN '".$startdate."' AND '".$enddate."')yield,
    ...
    (((SELECT COALESCE(SUM(area), 0) FROM tbl_harvesting as pl WHERE pl.mun_id=mn.mun_id AND  date_monitored BETWEEN '".$startdate."' AND '".$enddate."')/(SELECT target FROM tbl_target as tr WHERE tr.mun_id=mn.mun_id AND tr.program='HARVESTING' AND tr.season='".$_GET['season']."' AND tr.year='".$_GET['year']."' ))*100) percentageM
    FROM tbl_municipality as mn WHERE mn.district='".$x."' ORDER BY  mn.mun_id ASC;");
       
        while($row=mysqli_fetch_array($queryR)){
            //$x=$row1['area']/$row1['target'];
           // $percentage=$x*100;
            $tbl.='<tr>';
            $tbl.='<td>'.$row['municipality'].'</td>'; 
            $tbl.='<td>'.number_format($row['target'],2).'</td>';
            $tbl.='<td>'.number_format($row['area'],2).'</td>';
            $tbl.='<td>'.number_format($row['yield'],2).'</td>';
            $tbl.='<td>'.number_format($row['production'],2).'</td>';
